...
function initOmniDropdowns() {
    ...
    setupCustomSelect(selFrom);
    setupCustomSelect(selTo);
...
function setupCustomSelect(selElmnt) {
    ...
    for (let j = 0; j < selElmnt.length; j++) {
        ...
        let opt = selElmnt.options[j];
        let c = document.createElement("DIV");
        
        // Handle optgroups
        if (opt.parentNode.tagName === "OPTGROUP") {
            // Only add the label if this is the first option in the group
            if (opt === opt.parentNode.firstElementChild) {
                let labelDiv = document.createElement("DIV");
                labelDiv.setAttribute("class", "optgroup-label");
                labelDiv.innerHTML = opt.parentNode.label;
        ...
        c.addEventListener("click", function(e) {
            ...
            // Trigger change event for OmniConversion

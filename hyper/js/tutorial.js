class Tutorial {
    ...
    createOverlay() {
        ...
        if (!this.activeElement) {
            // Some elements might only exist if a track is created.
            // Let's create a dummy track if editor elements are missing.
            if (step.element.startsWith('edit-') || step.element === 'drawer-canvas') {
                ...
                // If it STILL doesn't exist after 200ms, just skip this step
                setTimeout(() => {
                    this.activeElement = document.getElementById(step.element) || document.querySelector(step.element);
                    if (this.activeElement) {
                        this.showStep();
        ...
        // Wait a tiny bit for scroll to settle before computing rect
        setTimeout(() => {
            const rect = this.activeElement.getBoundingClientRect();
            
            // Punch a hole in the overlay using clip-path
            const p = 8; // padding
        ...
        if (step.element === 'btn-add-track' || step.element === 'btn-add-track-side') {
            ...
            textEl.innerHTML = step.text + '<br><br><strong>(Click the Add Layer button to proceed)</strong>';
            ...
            this.tooltipBox.style.top = `${topPosBox}px`;
            this.tooltipBox.style.left = `${leftPosBox}px`;
        }, 100);
    }
